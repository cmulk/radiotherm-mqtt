...
class CT50:
    ...
    def __init__(self, therm_address, error_delay=10, timeout=10):
    ...
    def api_get(self, cmd):
        try:
            ...
            print("Exception with api call: " + cmd)
            print(repr(ex))
            raise
    ...
    def update_status(self):
        while True:
            ...
            if r.status_code != 200:
                print(f"Thermostat responed with error {r.text}")
                return r.json
            ...
            try:
                ...
                stat["fan_status"] = self.CODE_TO_FAN_STATE[stat["fstate"]]
                ...
                if stat["hvac_status"] == "cool":
                    ...
                    stat["action"] = "heating"
                elif stat["hvac_status"] == "off" and stat["fan_status"] == "off":
                    stat["action"] = "idle"
                elif stat["hvac_status"] == "off" and stat["fan_status"] == "on":
                    stat["action"] = "idle"
                ...
                print("Error updating status, retry...")
                print(repr(ex))
                sleep(self.error_delay)
    ...
    def set_temp(self, new_temp):
        temp = float(new_temp)
        print(f"Setting target temp to {temp}")
        ...
        while True:
            try:
                ...
                print("Error setting temp, trying again...")
                print(repr(e))
                sleep(self.error_delay)
        if r.status_code != 200:
            print(f"Thermostat set temp responed with error {r.text}")

        return self.update_status()

    def set_mode(self, new_mode):
        print(f"Setting HVAC mode to {new_mode}")
        ...
        while True:
            try:
                ...
                print("Error setting HVAC mode, trying again...")
                print(repr(e))
                sleep(self.error_delay)

        if r.status_code != 200:
            print(f"Thermostat set mode responed with error {r.text}")

        return self.update_status()

    def set_fan(self, new_fan_mode):
        print(f"Setting fan to {new_fan_mode}")
        ...
        while True:
            try:
                ...
                print("Error setting fan mode, trying again...")
                print(repr(e))
                sleep(self.error_delay)

        if r.status_code != 200:
            print(f"Thermostat set fan responed with error {r.text}")

        return self.update_status()

    def set_hold(self, new_hold_mode):
        print(f"Setting hold mode to {new_hold_mode}")
        ...
        while True:
            try:
                ...
                print("Error setting hold mode, trying again...")
                print(repr(e))
                sleep(self.error_delay)

        if r.status_code != 200:
            print(f"Thermostat set hold responed with error {r.text}")
    ...
    def set_clock(self):
        ...
        while True:
            try:
                ...
                print("Error setting time, trying again...")
                print(repr(e))
                sleep(self.error_delay)

        if r.status_code != 200:
            print(f"Thermostat set time responed with error {r.text}")
